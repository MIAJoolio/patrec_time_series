...
class DFT_wave_decomposer(Base_decomposer):
    """Seasonality decomposition using Discrete Fourier Transform."""
    
    def __init__(self, n_freqs: Union[int, str] = 'auto', fs: float = 1.0,
                 min_power_ratio: float = 0.1, min_freq: Optional[float] = None,
    ...
    def decompose(self, data: np.ndarray) -> Decomposition_result:
        ...
        return Decomposition_result(
            component=seasonal,
            component_type=FE_component_type.SEASONAL,
...
class DWT_wave_decomposer(Base_decomposer):
...
# class Shapelet_wave_decomposer(Base_decomposer):
...
#     def decompose(self, data: np.ndarray, y: np.ndarray) -> Decomposition_result:
#         """
#         Args:
...
#         Returns:
#             Decomposition_result containing shapelet distances
#         """
#         start_time = time.time()
